...
> **OUTPUT CONTRACT**: Your FINAL output must be a JSON object with exactly four keys:
> `slug` (string), `prd_dir` (string), `prd_md` (string), `prd_json` (object).
> `prd_md` is the complete prd.md file content as a string. `prd_json` is a native
> JSON object (the parsed prd.json structure — do NOT stringify/escape it into a
> string field; nesting JSON-as-a-string inside JSON is unreliable for structured
> output and has caused generation failures).
> The workflow engine will write the files to disk — you do NOT need to call bash to write them.
> You MAY still use bash to read existing files during codebase exploration.
...
### Phase 0.5: FETCH THE REAL TICKET (if applicable — do this BEFORE anything else)

The task description above is often a paraphrase written by whoever kicked off
this run — it can drop or compress acceptance criteria. If it references an
issue key (pattern `[A-Z][A-Z0-9]+-[0-9]+`, e.g. `SAP-3560`) and this codebase
has `JIRA_BASE_URL`/`JIRA_EMAIL`/`JIRA_API_TOKEN` configured, fetch the REAL
ticket yourself and treat it — not the paraphrase — as the authoritative source
of acceptance criteria:

```bash
ISSUE_KEY={ISSUE_KEY} python3 - <<'PY'
...
request = urllib.request.Request(
  f"{os.environ['JIRA_BASE_URL'].rstrip('/')}/rest/api/3/issue/{os.environ['ISSUE_KEY']}?fields=*all",
...
Count every distinct requirement (Gherkin `Scenario:` blocks, bullet points, or
numbered acceptance criteria — whatever format the ticket uses). This count is
your target: **every one of them must be covered by at least one story's
`acceptanceCriteria` in Phase 5.4.** A PRD that silently narrows a 10-criteria
ticket down to 1 story is exactly the failure mode this step exists to prevent
— a later workflow gate will hard-fail the run if your PRD's total declared
acceptance-criteria count is lower than the real ticket's scenario count, so
under-scoping here costs a full re-generation, not a warning.

If no issue key is found, or Jira isn't configured, skip this step and proceed
using only the task description as given.
...
### 4.5 Full Coverage Is Mandatory (if a real ticket was fetched in Phase 0.5)

Map every requirement you counted in Phase 0.5 to at least one story's
`acceptanceCriteria`. It is fine — expected, even — to split a large ticket
into several stories (schema story, backend story, UI story, etc.); it is NOT
fine to only generate stories for the easiest or most obvious requirement and
drop the rest. If you're tempted to size down to "just the first one for now,"
don't — generate the full set of right-sized stories instead (Section 4.2).
...
- [ ] Every requirement from the real ticket (Phase 0.5), if one was fetched, is covered by some story's acceptance criteria

---

## Phase 5: GENERATE — Compose PRD File Contents

In this phase you produce the CONTENT of the two PRD files. The workflow engine writes them to disk — you do NOT need to call bash.
...
### 5.2 Set the PRD directory path

Set `prd_dir` = `.archon/ralph/{slug}` in your output JSON.

### 5.3 Compose prd.md

Produce the full prd.md content for the `prd_md` field in your output JSON.
...
### 5.4 Compose prd.json

Produce the `prd_json` field as a native JSON object (NOT a stringified/escaped
JSON string — put it directly as an object value in your output, same nesting
level as `prd_md`):
...
```json
{
  ...
  "allowedPaths": [
    "{glob pattern for the primary target file, e.g. apps/voice-ops/src/pages/voice-health-summary.jsx}",
    "{glob pattern for any component files you expect to touch, e.g. apps/voice-ops/src/components/**}",
    "{glob pattern for the e2e/visual test spec you expect to add or update, e.g. apps/voice-ops/tests/e2e/**}"
  ]
}
```

**`allowedPaths` is a hard scope fence** — it will be used later to BLOCK the run
(before anything is pushed or reported) if the implementation touches any file
that doesn't match one of these patterns. Be deliberate:
- Include the exact target file(s) the user named.
- Include any component/util files you can already tell will need changes from
  your codebase exploration.
- Include the test directory pattern for the relevant app (e.g. `apps/voice-ops/tests/e2e/**`)
  so the implementer is free to add/update visual QA tests.
- Do NOT include a broad pattern like `apps/**` or `**` — that defeats the purpose.
  Prefer specific files over directories; use a directory glob only when you are
  genuinely uncertain which file(s) within it will need to change.

### 5.5 No file writing needed

The workflow engine writes the files for you after you return the JSON output.
...
- [ ] `allowedPaths` lists specific files/dirs, not broad wildcards
- [ ] Files committed
